...
void configQuickGame(){
  ...
  cls();
  //GAME TIME
  if(sdStatus || doStatus || saStatus || cwStatus){
    menu1:
    cls();
    lcd.print(GAME_TIME);
    delay(100);
  ...
  //sound??
  if(sdStatus || saStatus || doStatus || cwStatus){
    cls();
...
//menu ativar sirene bomba armada desarmada #teste
if(sdStatus || saStatus || cwStatus){
    cls();
    lcd.print(ENABLE_TESTEPIN);
    lcd.setCursor(0,1);
    lcd.print(YES_OR_NOT);
    while(1)
    {
